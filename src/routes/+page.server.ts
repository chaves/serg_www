...
async function fetchData(url: string, fetchFn: typeof fetch) {
	...
	if (!response.ok) {
		throw error(502, `Failed to fetch data from ${url}`);
	}
...
export const load: PageServerLoad = async ({ fetch }) => {
	...
	const dataNews: NewsResponse = await fetchData(
		`${CMS_BASE_URL}/api/news?publicationState=live&sort=createdAt:desc`,
		fetch
	);
	const dataEvents: EventsResponse = await fetchData(
		`${CMS_BASE_URL}/api/events?publicationState=live&sort=date_start:desc`,
		fetch
	);
